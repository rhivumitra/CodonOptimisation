...
def evaluate(model, loss_wrapper, dataloader, tokenizer, device):
    ...
    for step, (input_ids, attention_mask, labels) in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        for pred_nt, true_nt in zip(pred_nts, true_nts):
            ...
            print("AA:", aa_seq)  # show amino acid sequence used for generation
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for i in range(10):
            ...
            pred_nt = tokenizer.translate_aa_into_nt_torch(
                loss_wrapper, [aa_seq], return_string=True, batch_size=1, device=device, temperature=0.8
            )[0]
    ...
    # Train
    train_model(model, tokenizer, dataloader_provider, epochs=25, output_dir="hyena_checkpoints")
