...
from evo.cache import InferenceParams, RecurrentInferenceParams
from evo.engine import HyenaInferenceEngine
from evo.layers import ParallelGatedMLP, RMSNorm, VocabParallelEmbedding
from evo.utils import column_split, print_rank_0
...
class AttentionBlock(nn.Module):
    def __init__(self, config, layer_idx) -> None:
        ...
        self.proj_groups = config.proj_groups
        dtype = config.attn_block_dtype
        mlp_dtype = config.mlp_dtype
        ...
        self.inner_mha_cls = MHA(
            ...
            causal=True,
            layer_idx=layer_idx,
            out_proj_bias=config.mha_out_proj_bias,
            use_flash_attn=self.config.use_flash_attn,
        ).to(dtype=dtype)
        
        # check if using interpolated rotary pos emb from config, and swap the rope emb
        if config.use_interpolated_rotary_pos_emb:
            swap_mha_rope(
                mha=self.inner_mha_cls,
                kwargs_new_rope={'scaling_factor': config.rotary_emb_scaling_factor},
            )

        if self.config.smeared_gqa:
...
class ParallelHyenaFilter(nn.Module):
    def __init__(self, config, layer_idx) -> None:
        ...
        self.config = config
        self.layer_idx = layer_idx
        self.hyena_filter_groups = config.hyena_filter_groups

        self.use_flashfft = config.use_flashfft
        ...
        self.use_flash_depthwise = config.use_flash_depthwise
        ...
        if self.use_flash_depthwise:
            self.fir_fn = FlashDepthwiseConv1d(
                ...
                device=None,
                dtype=self.config.depthwise_dtype,
            )
        else:
            self.fir_fn = F.conv1d

        self.fftconv_fn = None
        self.long_fir_threshold = config.long_fir_threshold
    ...
    def parallel_forward(self, u, inference_params=None, padding_mask=None):
        ...
        y = self.engine.parallel_iir(
            ...
            prefill_style=self.config.prefill_style,
...
class ParallelGatedConvBlock(nn.Module):
    def __init__(self, config, layer_idx) -> None:
        ...
        self.config = config
        self.layer_idx = layer_idx
        self.low_mem_mode = config.low_mem_mode
        dtype = config.hyena_block_dtype
        mlp_dtype = config.mlp_dtype
...
class StripedHyena(nn.Module):
    def __init__(self, config):
        ...
        self.norm = RMSNorm(config) if config.final_norm else None
    ...
    def load_from_split_converted_state_dict(self, path):
        ...
        if self.config.final_norm is not None:
